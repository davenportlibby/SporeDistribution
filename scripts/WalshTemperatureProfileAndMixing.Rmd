...
WB.combined = ggplot() + geom_line(df.WB,mapping=aes(x=time,y=value, colour = factor(series)), size=.75) +
  ...
  scale_x_datetime(breaks = as.POSIXct(as.Date(c("2021-06-14", "2021-06-28","2021-07-12","2021-07-26","2021-08-09","2021-08-23","2021-09-07","2021-09-20","2021-10-04","2021-10-18","2021-11-01","2021-11-12"))), labels =
                     date_format("%m/%d"), 
                   limits = c(as.POSIXct(as.Date("2021-06-07")), as.POSIXct(as.Date("2021-11-20"))))+
  labs(colour = "Depth")
WB.combined
```
...
Wd <- strptime(Walsh$Date, format = "%m/%d/%y")
...
CVwalsh <- ggplot(data=Walsh,aes(x=WalshDate,y=CV, fill=mix, group = r)) +
  geom_boxplot()+
  geom_jitter(width = .15, height = .1, color="black", size=.8, alpha=1) +
  scale_x_datetime(breaks = as.POSIXct(as.Date(c("2021-06-14", "2021-06-28","2021-07-12","2021-07-26","2021-08-09","2021-08-23","2021-09-07","2021-09-20","2021-10-04","2021-10-18","2021-11-01","2021-11-12"))), labels =
                     date_format("%m/%d"), 
                   limits = c(as.POSIXct(as.Date("2021-06-07")), as.POSIXct(as.Date("2021-11-20"))))+
  ...
  labs(x = ("Date"), y=("Coefficient of Variation"),fill=("Depth"))+
  theme(plot.title = element_text(face = "bold",size = 12)) +
  theme(axis.text=element_text(size=8, face = "bold"), 
        axis.title=element_text(size=10,face="bold")) +
...
CVwalshMix <- ggplot(data=df1,aes(x=mix,y=CV, fill=mix)) +
  ...
  labs(x = ("Mixing Depth (m)"), y=("Patchiness"), fill=("Panel A: Depth \nPanel B: Mixing Depth"))+
  theme(plot.title = element_text(face = "bold",size = 12)) +
  theme(axis.text=element_text(size=8, face = "bold"), 
        axis.title=element_text(size=10,face="bold")) +
  theme(legend.position = "bottom")+
...
##combined=annotate_figure(combined,top = text_grob("Walsh Lake", color = "black", face = "bold", size = 12))

##combined

#### saving combined plot
##ggsave(here("figures", "Revised_Walsh_Combined_TemperatureProfile_Patchiness_Mixing.jpg"), combined, width = 8, ##height = 10, dpi = 600)
```

###New combined plot
```{r,message=F,warning=F}

leg2 <- get_legend(CVwalshMix)
combined2 = ggarrange(WB.combined, CVwalsh, ggarrange(walsh.CV.plot, leg2, ncol=2),
                     nrow = 3, labels = c("A.","B.","C."),
                     heights = c(1.5,1,1),
                     common.legend = FALSE,
                     legend="none")
...
combined2=annotate_figure(combined2,top = text_grob("Walsh Lake", color = "black", face = "bold", size = 12))

combined2

#### saving combined plot
ggsave(here("figures", "Walsh_Combined_Temperature_Regression.jpg"), combined2, width = 8, height = 10, dpi = 600)
```
